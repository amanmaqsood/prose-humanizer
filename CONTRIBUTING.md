...
- A behavioral eval case that catches lost facts, invented claims, voice flattening, semantic reversal, or unsafe file edits
...
3. Run `npm run validate` and `npm run benchmark`.
...
Benchmark fixtures must be synthetic, public domain, or explicitly licensed for redistribution. Record the source category in the pull request. Never commit private voice samples, generated local ballots, or candidate identity keys.
